...
---@class EcsClassData
---@field name string
---@field parent EcsClassData

---@class EcsBaseClass
---@field __class  EcsClassData


---@generic T
---@param class T
---@return T
function CLASS.new_instance(class, ...)
...
local function removei(t, value)
	for i = 1, #t do
		if t[i] == value then
...
function World:manage_systems()
    ...
    for i = 1, #s2r do
        ...
        if system.update then
            removei(self.systems_with_update, system)
        end
        if system.draw then
            removei(self.systems_with_draw, system)
        end
        removei(systems, system)
...
function World:draw(dt)
    ...
    for i = 1, #systems do
        ...
        draw(system, dt)
        --#IF DEBUG
        system.__time.current = get_time() - start_time
